...
bool TipoGestion::ComprobarNumVuelos(TipoVectorVuelos arrayVuelos) { // Procedimiento para comprobar si se ha llegado al máximo de vuelos permitidos en total
...
void TipoGestion::AltaNuevoVuelo(TipoVectorVuelos arrayVuelos) {
  ...
  if (ComprobarNumVuelos(arrayVuelos)){  // comprueba si se ha llegado al número máximo de vuelos permitidos
    do{
      try {
        system("cls");
        printf("\n Nuevo Vuelo:\n\n");
        ...
        if ((vueloAux.fecha.dia < 1) || (vueloAux.fecha.dia > 31)
          ...
          || ((vueloAux.fecha.dia > 30) && (vueloAux.fecha.mes == 11))) {       // Comprueba que los datos introducidos son correctos
        throw 1;
        }
